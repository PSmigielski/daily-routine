...
const fetchData = async () =>{
    ...
    browser.close();
    const countries = Array.from(new Set(data[1].map((c: ICountry) => c.name))).map(
        (name) => {
            return data[1].find((c: ICountry) => c.name === name);
        },
    );
     const timezones: ITimezone[] = Array.from(new Set(data[0].map((c) => c.name))).map(
         (name) => {
             return data[0].find((c) => c.name === name);
         },
     ) as ITimezone[];
