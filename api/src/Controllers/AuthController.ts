...
class AuthController {
  ...
  @Methods("/login", MethodsEnum.POST, [
  ...
  public async verify(req: Request, res: Response, next: NextFunction) {
    ...
    const result = await this.authService.verify(requestId).catch(next);
    if (result) {
      return res
        .status(202)
  ...
  public async refreshBearerToken(
    ...
    if (req.cookies.REFRESH_TOKEN != undefined) {
      const newToken = await this.authService
        .refreshBearerToken(
          req.cookies.REFRESH_TOKEN.token,
          req.ipData as IIpData,
        )
        .catch(next);
  ...
  @Methods("/reset/:requestId", MethodsEnum.PUT, [
    checkUuid("requestId"),
  ...
  public async sendResetRequest(
    ...
    const result = await this.authService.sendResetRequest(email).catch(next);
    if (result) {
      return res.json({ message: "reset request has been sent" });
    }
  }
  @Methods("/forget", MethodsEnum.POST, [
  ...
  @Methods("/edit/login", MethodsEnum.PUT, [
    checkJwt,
  ...
  public async editLogin(req: Request, res: Response, next: NextFunction) {
    ...
    if (result) {
      const result2 = await this.authService
      ...
      if (result2) {
        return res
          ...
          .status(202)
          .json({
            message: "You must sign in to complete the login change",
          });
      }
    }
  }

  @Methods("/edit/password", MethodsEnum.PUT, [
    checkJwt,
