...
class SubtaskController {
	...
	@Methods("/:subtaskId", MethodsEnum.PUT, [
		checkJwt,
		checkUuid("subtaskId"),
		schemaValidator("/../../schemas/subtask.schema.json"),
	])
	...
	@Methods("/:subtaskId", MethodsEnum.DELETE, [
		checkJwt,
		checkUuid("subtaskId"),
	])
	public async removeSubtask(
		...
		if (
			...
			const removedTask =
				await Subtask.removeSubtask(subtaskId).catch(next);
			if (removedTask) {
				return res.status(200).json({
	...
	@Methods("/mark/:subtaskId", MethodsEnum.PUT, [
		checkJwt,
		checkUuid("subtaskId"),
	])
