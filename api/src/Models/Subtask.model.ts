...
class Subtask extends Model {
	...
	public async createSubtask() {
		const subtask = await this.prisma.subtask
			.create({
				data: {
					name: this.name,
			...
			.catch((err) => {
				throw PrismaException.createException(err, "Subtask");
			});
		return subtask;
	}
	public static async getSubtasks(task: TaskType, page: number) {
		const limit = 25;
		const count = await this.prisma.subtask
			.count({ where: { task } })
			.catch((err) => {
				throw PrismaException.createException(err, "Subtask");
			});
		...
		const subtasks = await this.prisma.subtask
			.findMany({
				take: limit,
				skip: page * limit,
				where: { task },
	...
	public static async getSubtaskById(subtaskId: string) {
		const subtask = await this.prisma.subtask
			.findUnique({ where: { id: subtaskId } })
			.catch((err) => {
				throw PrismaException.createException(err, "Task");
			});
		if (!subtask) {
			throw new ApiErrorException(
				"Subtask with this id does not exist!",
				404,
			);
		}
		return subtask;
	}
	public static async checkOwnerOfTheSubtask(
		subtaskId: string,
		userId: string,
	) {
	...
	public static async editSubtask(subtaskId: string, name: string) {
		const updatedSubtask = await this.prisma.subtask
			.update({
				where: { id: subtaskId },
				data: { name },
			})
	...
	public static async removeSubtask(subtaskId: string) {
		const removedSubtask = await this.prisma.subtask
	...
	public static async checkIfAllSubtaskAreDone(taskId: string) {
		const allSubtasksInTaskCount = await this.prisma.subtask
			.count({ where: { taskId } })
			.catch((err) => {
				throw PrismaException.createException(err, "Subtask");
			});
		const allDoneSubtaskInTaskCount = await this.prisma.subtask
			.count({ where: { taskId, isDone: true } })
			.catch((err) => {
				throw PrismaException.createException(err, "Subtask");
			});
	...
	public static async markTaskAsDoneOrUndone(
		...
		if (await Task.checkOwnerOfTheTask(subtask.taskId, userId)) {
			...
			const isAllSubtasksDone = await Subtask.checkIfAllSubtaskAreDone(
				subtask.taskId,
			);
			if (isAllSubtasksDone) {
				const task = await Task.markTaskAsDoneOrUndone(
					subtask.taskId,
					userId,
					isAllSubtasksDone,
				);
